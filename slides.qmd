...
- TensorFlow (TF) includes the IMDB dataset in a vectorized format

:::

## Overview of Embeddings Neural Network {.smaller}

::: {.nonincremental}

- Neural network model implemented and trained using TF's Keras package
- TF and Keras Python implementations accessed from R via Reticulate package
...
## Neural Network Implementation In Keras {.smaller}
...
- Output of embedding layer is the same as the dimensionality hyperparameter
...
- Dense layer has a 1 output unit with sigmoid activation function that receives the flattened embedding layer output
...
- Dimensionality selection often done ad hoc with a functionality test
- We tested 2 through 7 dimensions
- Models compared by testing accuracy and qualitatievly looking at closest words
