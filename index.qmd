...
```{r}
# Use dummy_cols from fastDummies package for one-hot encoding
train_one_hot <- dummy_cols(train, select_columns = "sentiment", remove_first_dummy = FALSE)
train_one_hot <- train_one_hot[ , !names(train_one_hot) %in% c("sentiment")]

test_one_hot <- dummy_cols(test, select_columns = "sentiment", remove_first_dummy = FALSE)
test_one_hot <- test_one_hot[ , !names(test_one_hot) %in% c("sentiment")]

#append the one hot encoding to the cleaned text
train_one_hot <- train_one_hot %>% select(-text)
test_one_hot <- test_one_hot %>% select(-text)

train_processed <- cbind(tweets_train, train_one_hot)
test_processed <- cbind(tweets_test, test_one_hot)
```
