...
vocab_sorted <- vocab[order(vocab$term_count, decreasing=TRUE),] %>% mutate(idx = row_number()+3) # add 3 to skip over the start sequence token 1 and unknown word token 2
vocab_sorted <- head(vocab_sorted, n=max_vocab_words-4) # take only the top most common words, -4 to not include padding, start sequence and unknwon word tokens
...
vectorize_row <- function(r) {
  ...
  # add 1 to the start of every tweet for the start sequence character
  vectorized <- c(1, sapply(tokenized, function(x) vocab_translate[x]), rep(0,max_words-length(tokenized)))
  return(vectorized)
}
...
tokenized_train[is.na(tokenized_train)] <- 2 # any word outside our top vocab is NA, so replace with 2 for unknown word
...
tokenized_test[is.na(tokenized_test)] <- 2 # any word outside our top vocab is NA, so replace with 2 for unknown word
...
for (num_dims in rep(1:vector_dimensions)) {
...
names(accuracy_by_dimensions) <- rep(1:vector_dimensions)
accuracy_by_dimensions
```

```{r}
# Now that we know how many dimensions gives the greatest accuracy, pick number of epochs so model doesn't overfit

imdb_embedding_model <- keras_model_sequential() %>%
  layer_embedding(input_dim = max_vocab_words, output_dim = 2) %>%
  layer_flatten() %>%
  layer_dense(units = 1, activation = "sigmoid")
...
```{r}
# val_acc drops at epoch 6, so stop at epoch 5 to avoid overfitting
best_num_epochs <- 5
```

```{r}
#line = imdb_train_x[1,]
#imdb_translate = data.frame(word=names(dataset_imdb_word_index()), 
```
